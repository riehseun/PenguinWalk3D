...
void initGlui()
{
	...
	glui_spinner->set_speed(SPINNER_SPEED*10); // for fast moving beak
	...
	glui_spinner->set_float_limits(SHOULDER_PITCH_MIN, 0, GLUI_LIMIT_CLAMP);
	glui_spinner->set_speed(SPINNER_SPEED);
	...
	glui_spinner->set_float_limits(0, SHOULDER_PITCH_MAX, GLUI_LIMIT_CLAMP);
	glui_spinner->set_speed(SPINNER_SPEED);
	...
	switch (renderStyle) {
		...
		case METALLIC: {
			...
			GLfloat light_position[] = {
				joint_ui_data->getDOF(Keyframe::LIGHT_X),
				joint_ui_data->getDOF(Keyframe::LIGHT_Y),
				20.0,
				30.0
			};
			...
			GLfloat metallic_ambient[] = {1.0, 1.0, 1.0},
			metallic_diffuse[] = {1.0, 1.0, 1.0},
			metallic_specular[] = {1.0, 1.0, 1.0},
			metallic_shininess = 16;
			glMaterialfv(GL_FRONT, GL_AMBIENT, metallic_ambient);
			glMaterialfv(GL_FRONT, GL_DIFFUSE, metallic_diffuse);
			glMaterialfv(GL_FRONT, GL_SPECULAR, metallic_specular);
			glMaterialf(GL_FRONT, GL_SHININESS, metallic_shininess);
		...
		case MATTE: {
			...
			GLfloat matte_ambient[] = {0.1, 0.1, 0.1},
			matte_diffuse[] = {0.1, 0.1, 0.1},
			matte_specular[] = {0.1, 0.1, 0.1},
			matte_shininess =  128;
			glMaterialfv(GL_FRONT, GL_AMBIENT, matte_ambient);
			glMaterialfv(GL_FRONT, GL_DIFFUSE, matte_diffuse);
			glMaterialfv(GL_FRONT, GL_SPECULAR, matte_specular);
			glMaterialf(GL_FRONT, GL_SHININESS, matte_shininess);
...
void penguin() {
	glPushMatrix();
		...
		// setup transformation for arms-hands parts
		glPushMatrix();
			glScalef(0.5,0.5,1); 
			glTranslatef(0,0,1); // translate to right
			glTranslatef(0,1,0); // translate to pivot location
			...
			glTranslatef(0,-1,0); // translate to final posision
			arm(); // draw right arm
			glPushMatrix();
				glScalef(1,0.5,0.5);
				glTranslatef(0,-2,0); // translate to pivot location
				glRotatef(-joint_ui_data->getDOF(Keyframe::R_ELBOW), 0.0, 0.0, 1.0);
				glTranslatef(0,-1,0); // translate to final posision
				hand(); // draw right hand
			glPopMatrix();
		glPopMatrix();
		glPushMatrix();
			glScalef(0.5,0.5,1);
			glTranslatef(0,0,-1); // translate to left
			glTranslatef(0,1,0); // translate to pivot location
		...
		// setup transformation for head-beaks parts
		glPushMatrix();
			glScalef(1,0.25,1);
			glTranslatef(0,4,0); // translate to pivot location
			glRotatef(joint_ui_data->getDOF(Keyframe::HEAD), 0.0, 0.0, 1.0);
			glTranslatef(0,1,0); // translate to final posision
			body(); // draw head
			glPushMatrix();
			...
			glPushMatrix();
				...
				//glTranslatef(joint_ui_data->getDOF(Keyframe::BEAK), 0, 0);
				//glTranslatef(0,0,joint_ui_data->getDOF(Keyframe::BEAK));
				beak(); // draw upper beak
			glPopMatrix();
		glPopMatrix();

		// setup transformation for legs-feet parts	
		glPushMatrix();
			glScalef(0.1,0.5,0.1);
			glTranslatef(1,-2,5); // translate to pivot location
			...
			glPushMatrix();
				glScalef(5,0.5,4);
				glTranslatef(-1,-2,0); // translate to pivot location and final position
				glRotatef(joint_ui_data->getDOF(Keyframe::R_KNEE), 0.0, 0.0, 1.0);
				foot(); // draw right foot
			glPopMatrix();
		glPopMatrix();
		glPushMatrix();
			glScalef(0.1,0.5,0.1);
